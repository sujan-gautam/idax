...
const AiChat: React.FC<AiChatProps> = ({ datasetId, projectId, compact = false }) => {
    ...
    return (
        <Card className={cn(
            "transition-all duration-300 border-slate-200 dark:border-slate-800 flex flex-col shadow-xl overflow-hidden relative",
            isExpanded ? "h-[640px]" : "h-[70px]"
        )}>
            <CardHeader className="p-4 border-b dark:bg-slate-900/50 flex flex-row items-center justify-between space-y-0 shrink-0">
                ...
                <div className="flex items-center gap-1">
                    {isExpanded && (
                        <>
                            <Button variant="ghost" size="icon" onClick={startNewChat} title="New Chat" className="h-8 w-8 text-slate-500">
                                <PlusCircle className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => setShowHistory(!showHistory)} title="History" className={cn("h-8 w-8", showHistory ? "text-indigo-600 bg-indigo-50" : "text-slate-500")}>
                                <History className="h-4 w-4" />
                            </Button>
                            <div className="w-[1px] h-4 bg-slate-200 dark:bg-slate-800 mx-1" />
                        </>
                    )}
                    <Button variant="ghost" size="icon" onClick={() => { setIsExpanded(!isExpanded); setShowHistory(false); }} className="h-8 w-8">
                                    ...
                                    {msg.role === 'assistant' ? (
                                        <ReactMarkdown
                                            components={{
                                                ...
                                                h3: ({ children }) => <h3 className="text-sm font-black mt-4 mb-2 uppercase tracking-wide text-slate-900 dark:text-slate-100">{children}</h3>,
                                                strong: ({ children }) => <strong className="font-bold text-indigo-600 dark:text-indigo-400">{children}</strong>,
                                                code: ({ children }) => <code className="bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded text-[11px] font-mono text-indigo-600 dark:text-indigo-400">{children}</code>
                                            }}
                                        >
                                            {msg.content}
                                        </ReactMarkdown>
