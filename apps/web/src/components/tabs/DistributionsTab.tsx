...
export const DistributionsTab: React.FC<DistributionsTabProps> = ({ datasetId }) => {
    ...
    const renderNumericDistribution = (colName: string, data: any) => {
        ...
        const chartData = data.histogram.counts.map((count: number, idx: number) => ({
            ...
            count,
            density: count / data.histogram.counts.reduce((a: number, b: number) => a + b, 0),
        }));
                    ...
                    {data.entropy !== undefined && (
                        <div className="mt-4 rounded-lg bg-indigo-50 p-4 dark:bg-indigo-900/20">
                            <div className="text-sm font-medium text-indigo-900 dark:text-indigo-100">
                                Shannon Entropy: {data.entropy.toFixed(3)}
                            </div>
                            <div className="text-xs text-indigo-700 dark:text-indigo-300">
                                Measures the randomness/diversity of categorical values
                            </div>
                        </div>
                    )}
                </CardContent>
